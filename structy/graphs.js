...
const semestersRequired = (numCourses, prereqs) => {
  const semGraph = makeGraph(numCourses, prereqs);
  const distance = {}; 
  
  for (let i = 0; i < numCourses; i+=1) {
    if (semGraph[i].length === 0) distance[i] = 1;
  }
  
  for (let i = 0; i < numCourses; i += 1) {
    determineSems(semGraph, i, distance)
  }
  
  return Math.max(...Object.values(distance))
};

const determineSems = (graph, node, distance) => {
  if (node in distance) return distance[node];
  
  let maxDistance = 0; 
  
  for (let neighbor of graph[node]) {
    const neighborDistance = determineSems(graph, neighbor, distance);
